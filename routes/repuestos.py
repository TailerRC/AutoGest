...
def render_repuestos_list(req, usuario, repuestos, todos_repuestos, q="", filtro_stock="todos", orden="nombre_asc", page=1, total_pages=1, total_count=0):
    ...
    alertas_compactas_grid = Div(
        Div(
            Div(H2(I(cls="fa-solid fa-triangle-exclamation"), " Stock Crítico"), cls="card-header"),
            Div(
                Div(
                    Div(
                        Canvas(id="chartCritico", style="max-height:160px;max-width:160px;"),
                        Div(
                            Div(str(len(criticos)), style="font-size:1.6rem;font-weight:800;color:var(--red);line-height:1;"),
                            Div("críticos", style="font-size:.7rem;color:var(--text-muted);font-weight:500;"),
                    ...
                    Div(
                        Div(
                            Div(style="width:10px;height:10px;border-radius:3px;background:var(--red);flex-shrink:0;"),
                            Div("Crítico (≤2)", style="font-size:.8rem;color:var(--text-secondary);"),
                            style="display:flex;align-items:center;gap:.5rem;"
                        ),
                        Div(
                            Div(style="width:10px;height:10px;border-radius:3px;background:var(--border-strong,#d1d5db);flex-shrink:0;"),
                            Div("Resto del inventario", style="font-size:.8rem;color:var(--text-secondary);"),
                            style="display:flex;align-items:center;gap:.5rem;"
                        ),
                ...
                cls="card-body", style="padding:1.5rem;"
            ),
            cls="card fade-in"
        ),
        Div(
            Div(H2(I(cls="fa-solid fa-bolt"), " Stock Bajo"), cls="card-header"),
            Div(
                Div(
                    Div(
                        Canvas(id="chartBajo", style="max-height:160px;max-width:160px;"),
                        Div(
                            Div(str(len(bajos)), style="font-size:1.6rem;font-weight:800;color:var(--yellow);line-height:1;"),
                            Div("bajos", style="font-size:.7rem;color:var(--text-muted);font-weight:500;"),
                            style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;"
                        ),
                        style="position:relative;display:flex;align-items:center;justify-content:center;"
                    ),
                    Div(
                        Div(
                            Div(style="width:10px;height:10px;border-radius:3px;background:var(--yellow);flex-shrink:0;"),
                            Div("Bajo (≤5)", style="font-size:.8rem;color:var(--text-secondary);"),
                            style="display:flex;align-items:center;gap:.5rem;"
                        ),
                        Div(
                            Div(style="width:10px;height:10px;border-radius:3px;background:var(--border-strong,#d1d5db);flex-shrink:0;"),
    ...
    total_repuestos = len(todos_repuestos)
    resto_criticos = total_repuestos - len(criticos)
    resto_bajos = total_repuestos - len(bajos)

    charts_stock_script = Script(f"""
        document.addEventListener('DOMContentLoaded', function() {{
            new Chart(document.getElementById('chartCritico').getContext('2d'), {{
                type: 'doughnut',
                data: {{
                    labels: ['Crítico', 'Resto'],
                    datasets: [{{
                        data: [{len(criticos)}, {resto_criticos}],
            ...
            new Chart(document.getElementById('chartBajo').getContext('2d'), {{
                type: 'doughnut',
                data: {{
                    labels: ['Bajo', 'Resto'],
                    datasets: [{{
                        data: [{len(bajos)}, {resto_bajos}],
